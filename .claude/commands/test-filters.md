...
- **Test sub-category pages, not just main pages.** Brand-filtered pages (e.g., `/tvs/sony`) recalculate filter visibility from the filtered product set. If a filter group relies on generic text matching and the product spec data is incomplete, ALL options in that group will show count=0 and be hidden — even if the reference site shows them. Always test at least one brand-filtered page per category.
- **Classify filter groups by matching reliability.** Some groups have dedicated handlers (Brand, Price, Type, Screen Size) that reliably match products. Others use generic text fallback (Colour, Voice control, Features, Smart TV apps) which fails when product specs are incomplete. Track which groups are "reliable" vs "unreliable" — unreliable groups should keep parent options on sub-category pages instead of being recalculated.
- **Live comparison is the gold standard.** Scrape 3+ reference site category pages live and compare every group and every option (including collapsed sections) against local data. This catches: missing groups entirely absent from scraped data, truncated labels, structural hub-vs-listing mismatches, and options that were in collapsed sections during the initial scrape.
- **Check collapsed sections explicitly.** During live comparison, verify the group count matches expectations. If a listing page should have 10+ filter groups but you only see 3-4, the scrape missed collapsed accordion sections. Re-scrape with longer wait times or different format.
