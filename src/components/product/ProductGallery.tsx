...
export default function ProductGallery({ images, thumbnails, alt }: ProductGalleryProps) {
  ...
  // Filter to only images that haven't failed
  const validIndices = images.map((_, i) => i).filter((i) => !failedImages.has(i));
  const validCount = validIndices.length;

  // If current image failed, jump to the first valid one
  const effectiveCurrent = failedImages.has(current)
    ? (validIndices[0] ?? 0)
    : current;

  const prev = () => {
    const pos = validIndices.indexOf(effectiveCurrent);
    const prevIdx = validIndices[(pos - 1 + validCount) % validCount];
    setCurrent(prevIdx);
  };

  const next = () => {
    const pos = validIndices.indexOf(effectiveCurrent);
    const nextIdx = validIndices[(pos + 1) % validCount];
    setCurrent(nextIdx);
  };
  ...
  return (
    <div>
      ...
      <div className="relative aspect-[4/3] bg-white border border-border rounded-[5px] overflow-hidden mb-3">
        <Image
          src={images[effectiveCurrent]}
          alt={`${alt} - Image ${effectiveCurrent + 1}`}
          fill
          className="object-contain p-4"
          priority
          onError={() => handleImageError(effectiveCurrent)}
        />
        {validCount > 1 && (
          <>
            <button
        ...
        <span className="absolute bottom-3 left-1/2 -translate-x-1/2 text-xs text-text-secondary bg-white/80 px-2 py-0.5 rounded">
          {validIndices.indexOf(effectiveCurrent) + 1} / {validCount}
        </span>
      </div>

      {/* Thumbnails — only show ones that loaded */}
          ...
          return (
            <button
              ...
              className={`w-[80px] h-[80px] flex-shrink-0 border rounded overflow-hidden ${
                i === effectiveCurrent ? "border-primary border-2" : "border-border"
              }`}
            >
              <Image
                src={thumb}
